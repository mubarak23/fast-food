...
export default (props) => {
  ...
  <button disabled={props.pagination && props.pagination.prev_page_url ? false : true} 
  onClick={() => { props.nextPage(props.pagination.prev_page_url) }}  
  className={`relative inline-flex items-center px-2 py-2 rounded-l-md 
  border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50
   ${ !props.pagination.prev_page_url ? "cursor-not-allowed": "cursor-pointer"}`}>
 Previous
    </button>
    <button disabled={props.pagination && props.pagination.next_page_url ? false : true} 
    onClick={() => props.nextPage(props.pagination.next_page_url)} 
    className={`relative inline-flex items-center px-2 py-2 rounded-r-md 
    border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 ${ !props.pagination.next_page_url ? "cursor-not-allowed": "cursor-pointer"}`}>
          Next
    </button>
  </div>
  <div className="hidden sm:flex-1 ml-32 sm:flex sm:items-center sm:justify-between my-16">
    {!props.loading && props.pagination && props.pagination.total > 0 && <div>
      <p className="text-sm px-2 text-gray-700">
        Showing
        <span className="font-medium px-1">{props.pagination && 
        ((Number(props.pagination.current_page) - 1) * 10) + 1}</span>
        to
        <span className="font-medium px-1">{props.pagination && 
        !props.pagination.next_page_url ? props.pagination.total : Number(props.pagination.current_page) * 10}</span>
        of
    ...
    <div>
      {!props.loading && props.pagination && props.pagination.total > 0 && 
      <nav className="relative z-0 inline-flex shadow-sm -space-x-px" aria-label="Pagination">
        <button disabled={props.pagination && props.pagination.prev_page_url ? false : true} 
        onClick={() => { props.nextPage(props.pagination.prev_page_url) }}  
        className={`relative inline-flex items-center px-2 py-2 rounded-l-md border
         border-gray-300 bg-white text-sm font-medium text-gray-500
          hover:bg-gray-50 ${ !props.pagination.prev_page_url ? "cursor-not-allowed": "cursor-pointer"}`}>
        ...
        border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50
         ${ !props.pagination.next_page_url ? "cursor-not-allowed": "cursor-pointer"}`}>
