...
const Order = (props) => {
    ...
    const config = {
        ...
        email: props.email || "mubarak23@example.com",
 ...
 <section>
    ...
    <section className="w-full xl:w-3/12 my-8 xl:my-6">
        ...
        <CartCheckout header={'Order Summary'} qty={props.cartItems && props.cartItems.length} 
        total={props.cart && props.cart.totalPrice} items={props.cart && props.cart.totalPrice} shipping={0} tax={0}>
        {button ?
        <button onClick={() => sendOrder(props.cart)} className={`w-full mt-4 flex justify-center border 
        border-gray-200 rounded py-4 px-4 leading-tight focus:outline-none focus:bg-white 
        focus:text-black focus:border-gray-500 hover:bg-white hover:text-black hover:border-gray-500 
        uppercase ${ props.loading ? "bg-gray-200  text-black cursor-not-allowed": "bg-gray-900  text-white cursor-pointer"}`}>
            {props.loading ?  (
                <>
        ...
        <PaystackButton {...componentProps} className={`w-full mt-4 flex justify-center border
         border-gray-200 rounded py-4 px-4 leading-tight focus:outline-none focus:bg-white 
         focus:text-black focus:border-gray-500 hover:bg-white hover:text-black hover:border-gray-500 
         uppercase ${ props.loading ? "bg-gray-200  text-black cursor-not-allowed": 
