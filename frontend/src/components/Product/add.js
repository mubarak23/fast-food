...
const AddProduct = (props) => {
    ...
    const handleEditorChange = (content, editor) => {
        let editorTxt = editor.getContent({ format: 'raw' })
        let decodeContent = decodeHtml(editorTxt)
        let stripedContent = decodeContent.replace(/(<([^>]+)>)/ig, "").trim();
        setDescription(content)
        setRealCharacters(stripedContent)
    ...
    const saveProduct = async({current:{value: name}}, {current:{value: price}}, description) => {
        ...
        let result = await props.addProduct(name, price, description, image)
        if(result) return refreshForm()
    }
...
<button className="my-2 py-2 flex justify-center items-center flex-col"id="product-image" aria-haspopup="true">
   ...
   {preview ? (
     ...
     <img title="Preview image" className=" h-44 w-44 rounded-full object-contain" 
     src={preview} alt="No preview avaliable"/>
   ) : (
    props.data && props.data.image ? (
      <img title="Preview image" className=" h-44 w-44 rounded-full object-contain"
       src={props.data.image} alt="No preview avaliable"/>

    ) : (

      <span title="Preview will display here" className="block h-44 w-44 mb-4 
      rounded-full bg-gradient-to-r from-gray-300 via-gray-300 to-gray-300"></span>
    )
   )}
  ...
  <div className="w-full border-t border-gray-200">
    ...
    <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
      ...
      <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
        ...
        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
        <button disabled={props.loading} className={`w-full mt-3 flex justify-center hover:bg-gray-200 
        hover:text-gray-900 rounded-md px-3 py-3 uppercase ${ props.loading ? 
