...
const Order = (props) => { 
    ...
    const refreshFilters = () => {
      ...
      const getGeneral = (current,incoming,demo) => {
        if(current){
          current.includes(incoming) ? demo(current.filter((e) => String(e) !== String(incoming))) 
          : demo([...current,incoming])
        }else{
          demo([incoming])
        }
      }
  ...
  <section className="px-4">
      ...
      <button onClick={() => { setFilter('clicked') }} className="w-full mt-12 mb-4 bg-gray-900
       md:w-1/4 border border-gray-200 text-white rounded py-4 px-4 leading-tight 
       focus:outline-none focus:bg-white focus:text-black focus:border-gray-500"> Apply Filters</button>
  </section>
  </div>
  )}
  </StyledProduct>
